...
df = pd.read_csv(
    "../data/aggregates/aus1league_physicalaggregates_20242025_midfielders.csv"
)
...
def get_similar_players(
    df, similarity_df, player_name, position_group, top_n=5, filter_col=None
):
    ...
    player_info = df[
        ["unique_id", "player_name", "team_name", "position_group"]
    ].set_index("unique_id")
...
def feature_contributions(
    df, player1, position_group1, player2, position_group2, feature_cols
):
...
position_group = st.selectbox(
    "Select the position/group for this player:", positions_available
)
...
top_n = st.slider(
    "Number of similar players to show:", min_value=1, max_value=20, value=5
)

# Step 5: compute similarity
if st.button("Find similar players"):
    similar_players = get_similar_players(
        df, similarity_df, player_name, position_group, top_n, filter_col=filter_option
    )
    ...
    if similar_players.empty:
        ...
        contribs = feature_contributions(
            ...
            player_name,
            position_group,
            similar_players.iloc[0]["player_name"],
            similar_players.iloc[0]["position_group"],
            feature_cols,
        )
        ...
        fig = px.bar(
            ...
            y="feature",
            orientation="h",
            title=f"Top 10 features contributing to similarity between {player_name}",
